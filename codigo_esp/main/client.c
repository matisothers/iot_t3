...
int Client__udp(struct Client* self){
    // Crear socket
    struct sockaddr_in server_addr;
...
int Client__handle(struct Client* self){ 
    char buffer[1024];
    ...
    while (1){
        ...
        if (transport_layer == 2){
            // Utilizar TCP
            Client__tcp(self);
            return transport_layer;
            //
        }
        if (transport_layer == 3){
            // Utilizar UDP
            transport_layer = Client__udp(self);
        }
        if ( transport_layer < 2 ){ // nos vamos a BLE
            return transport_layer;
        }
    }
}
