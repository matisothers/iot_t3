...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (strncmp((char *)param->write.value, expected_con_init_msg, 3) == 0) {
            ...
            tl = (char *)param->write.value[3];
            protocol = (char *)param->write.value[4];
            ESP_LOGI("MY_DEMO", "Protocolo: %c. Transport layer: %c", protocol, tl);
...
int run_server(void)
{
    ...
    while(tl == '0'){ // SOLO NO RETORNA SI ES BLE CONTINUO
